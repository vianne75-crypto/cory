...
| 대리점세금계산서-팝빌API구현 | SAGE 🟡 | FLUX(대표 방식A 확정). **[Phase0 착수 7/01]** ✅대리점 12명 명단 추출+`dealer_business_info` 스키마(사외비). **대표 확인 대기**: ①회원목록 진단 스니펫 실행→사업자번호 수집구조 확정 ②빈 mem_id 8건(680만·미발행7). **선행 블로커: 대표 팝빌 가입·인증서(Phase1)**. 이후 ②`issue_invoices_asp.py`+승인목록+발행결과 Supabase쓰기(서비스롤) | `cory/대리점_사업자번호_매핑_Phase0_FLUX_20260701.md` `경영지원(슬기)/재무/대리점_세금계산서_발행자동화_설계_v1.md` | 🟡 팝빌 세팅 후 | OPEN |
