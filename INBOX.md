...
| 대리점세금계산서-팝빌발행 | 대표 🟡 | **[팝빌 API 확정·발행대기 7/01]** 대표 팝빌+국세청위임 기보유 확인. `issue_invoices_asp.py`(popbill SDK) 완성·검증. **6월 4장 6,125,000원** 스코핑(현금만·카드제외, 기프트수림 김은주외1명 합산2줄, 작성일 6-30·거래일=입금일·단가공란). **유일 블로커: 팝빌 LinkID·SecretKey 발급 대기**(대표 신청함)→오면 테스트→실발행→invoice_issued 반영. **다음할일**: cory [발행]버튼→실팝빌발행 연동 | `cory/scripts/issue_invoices_asp.py` `_SECRET/대리점_세금계산서_발행목록_2026-06.md` | 🟡 팝빌키 대기 | OPEN |
