...
| 대리점세금계산서-팝빌발행 | 대표 🟡 | **[팝빌 API 확정·발행대기 7/01]** 대표 팝빌+국세청위임 기보유 확인. `issue_invoices_asp.py`(popbill SDK) 완성·검증. **6월 4장 6,125,000원** 스코핑(현금만·카드제외, 기프트수림 김은주외1명 합산2줄, 작성일 6-30·거래일=입금일·단가공란). ✅**테스트 발행 4장 성공+대표 시각확인**(연동·인증서·기프트수림 합산 전검증). ✅운영전환 **신청완료**(7/02)·팝빌 **승인대기**(+운영환경 공동인증서)→그후 IsTest=False 실발행→invoice_issued 반영. **다음할일**: cory [발행]버튼→실팝빌발행 연동 | `cory/scripts/issue_invoices_asp.py` `_SECRET/대리점_세금계산서_발행목록_2026-06.md` | 🟡 팝빌승인 대기 | OPEN |
