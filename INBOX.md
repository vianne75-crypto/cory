...
| 대리점세금계산서-팝빌API구현 | SAGE 🟡 | FLUX(대표 방식A 확정). **[✅Phase0 완료 7/01]** 애니빌드 API 전환(스크래퍼 폐기): `biz_num` 확인·Pull조회없음. Worker `/anybuild-member`(가입·수정 webhook 자동)+`/sync-dealer-business` 배포, `dealer_business_info`(사외비) **108명 백필**(사업자번호99). 신규·수정 영구 무인. **남음**: 누락9명+빈 mem_id 8건(680만·미발행7) 확인 · **Phase1 대표 팝빌가입=발행블로커** · Phase2 `issue_invoices_asp.py` | `cory/대리점_사업자번호_매핑_Phase0_FLUX_20260701.md` `경영지원(슬기)/재무/대리점_세금계산서_발행자동화_설계_v1.md` | 🟡 팝빌 세팅 후 | OPEN |
